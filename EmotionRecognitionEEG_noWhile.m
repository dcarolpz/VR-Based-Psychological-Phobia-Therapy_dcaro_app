...
%       4. Disgust. 
% -------------------------------------------------------------------------
% This version avoids the use of a "While true [...]" statement, to allow
% its implementation within an app.
...
% General parameters 
load 'Models/1_5-Nov-2025.mat' dcaro_net
fs = 250;
window_size = 6;    
recorded = 1;
...
dcaro_timer = timer('ExecutionMode','fixedSpacing','Period',6,'TimerFcn', ...
    @(~,~)dcaro_updates(inlet,window,recorded,dcaro_net,classes,fs,t));
start(dcaro_timer)

figure
function dcaro_updates(inlet,window,recorded,dcaro_net,classes,fs,t)
